...
def replace_lb_3D(U_bd, aux_bd, padded_U, padded_aux):
    U = padded_U.at[:,0:3,3:-3,3:-3].set(U_bd)
    aux = padded_aux.at[:,0:3,3:-3,3:-3].set(aux_bd)
    return U, aux

    
def replace_rb_3D(U_bd, aux_bd, padded_U, padded_aux):
    U = padded_U.at[:,-3:,3:-3,3:-3].set(U_bd)
    aux = padded_aux.at[:,-3:,3:-3,3:-3].set(aux_bd)
    return U, aux


def replace_ub_3D(U_bd, aux_bd, padded_U, padded_aux):
    U = padded_U.at[:,3:-3,-3:,3:-3].set(U_bd)
    aux = padded_aux.at[:,3:-3,-3:,3:-3].set(aux_bd)
    return U, aux

def replace_bb_3D(U_bd, aux_bd, padded_U, padded_aux):  
    U = padded_U.at[:,3:-3,0:3,3:-3].set(U_bd)
    aux = padded_aux.at[:,3:-3,0:3,3:-3].set(aux_bd)
    return U, aux

def replace_fb_3D(U_bd, aux_bd, padded_U, padded_aux):  
    U = padded_U.at[:,3:-3,3:-3,0:3].set(U_bd)
    aux = padded_aux.at[:,3:-3,3:-3,0:3].set(aux_bd)
    return U, aux

def replace_backb_3D(U_bd, aux_bd, padded_U, padded_aux):
    U = padded_U.at[:,3:-3,3:-3,-3:].set(U_bd)
    aux = padded_aux.at[:,3:-3,3:-3,-3:].set(aux_bd)
    return U, aux
