...
def interface_L_z(q,k=1/3,beta=1.0):
    
    U_z = q[:,3:-3,3:-3,1:-1]
    d_minus_z = U_z[:,:,:,1:-1]-U_z[:,:,:,:-2]
    d_plus_z = U_z[:,:,:,2:]-U_z[:,:,:,1:-1]
    
    phi_i_z_fw = min_mod_limiter(d_plus_z,beta*d_minus_z)
    phi_i_z_bw = min_mod_limiter(d_minus_z,beta*d_plus_z)

    s_i_L_z = 1/4*((1-k)*phi_i_z_bw + (1+k)*phi_i_z_fw)

    q_L_z = U_z[:,:,:,1:-2] + s_i_L_z[:,:,:,:-1]
...
def interface_R_z(q,k=1/3,beta=1.0):
    U_z = q[:,3:-3,3:-3,1:-1]
    d_minus_z = U_z[:,:,:,1:-1]-U_z[:,:,:,:-2]
    d_plus_z = U_z[:,:,:,2:]-U_z[:,:,:,1:-1]
    ...
    s_i_R_z = 1/4*((1-k)*phi_i_z_fw + (1+k)*phi_i_z_bw)
    
    q_R_z = U_z[:,:,:,2:-1] - s_i_R_z[:,:,:,1:]
