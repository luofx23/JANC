...
def inteface_mach_number(q_L_x,q_R_x,q_L_y,q_R_y,q_L_z,q_R_z,h_L_x,h_R_x,h_L_y,h_R_y,h_L_z,h_R_z,gamma_L_x,gamma_R_x,gamma_L_y,gamma_R_y,gamma_L_z,gamma_R_z):
    rho_L = q_L_x[0:1]
    u_L,v_L,w_L = q_L_x[1:2],q_L_x[2:3],q_L_x[3:4]
    V_square = (u_L)**2
    p_L_x = q_L_x[4:5]
    ...
    rho_R = q_R_x[0:1]
    u_R,v_R,w_R = q_R_x[1:2],q_R_x[2:3],q_R_x[3:4]
    V_square = (u_R)**2
    p_R_x = q_R_x[4:5]
    ...
    rho_L = q_L_y[0:1]
    u_L,v_L,w_L = q_L_y[1:2],q_L_y[2:3],q_L_y[3:4]
    V_square = (v_L)**2
    p_L_y = q_L_y[4:5]
    ...
    rho_R = q_R_y[0:1]
    u_R,v_R,w_R = q_R_y[1:2],q_R_y[2:3],q_R_y[3:4]
    V_square = (v_R)**2
    p_R_y = q_R_y[4:5]
    gamma_R = gamma_R_y#p_L_x/rhoe + 1 #rhoE shold mius kinetic!!!
    ...
    rho_L = q_L_z[0:1]
    u_L,v_L,w_L = q_L_z[1:2],q_L_z[2:3],q_L_z[3:4]
    V_square = (w_L)**2
    p_L_z = q_L_z[4:5]
    gamma_L = gamma_L_z#p_L_x/rhoe + 1 #rhoE shold mius kinetic!!!
    H_L = h_L_z + 0.5*V_square
    
    rho_R = q_R_z[0:1]
    u_R,v_R,w_R = q_R_z[1:2],q_R_z[2:3],q_R_z[3:4]
    V_square = (w_R)**2
    p_R_z = q_R_z[4:5]
    gamma_R = gamma_R_z#p_L_x/rhoe + 1 #rhoE shold mius kinetic!!!
    H_R = h_R_z + 0.5*V_square
    
    Hnormal = 0.5*(H_L + H_R)
    gamma = 0.5*(gamma_L+gamma_R)
    cs_z = jnp.sqrt(2*(gamma-1)/(gamma+1)*Hnormal)
    indicator_z = (1/2*(w_L+w_R)>0)
    dom_L = jnp.maximum(jnp.abs(w_L),cs_z)
    dom_R = jnp.maximum(jnp.abs(w_R),cs_z)
    c_L_z = jnp.sqrt(gamma_L*p_L_z/rho_L)
    c_R_z = jnp.sqrt(gamma_R*p_R_z/rho_R)
    c_interface_z = 0.5*(c_L_z+c_R_z)
    M_L_z = w_L/c_interface_z
    M_R_z = w_R/c_interface_z
	
    return M_L_x,M_R_x,M_L_y,M_R_y,M_L_z,M_R_z,c_interface_x,c_interface_y,c_interface_z
...
def riemann_flux(q_L_x,q_R_x,q_L_y,q_R_y,q_L_z,q_R_z):
    ...
    M_L_x,M_R_x,M_L_y,M_R_y,M_L_z,M_R_z,c_interface_x,c_interface_y,c_interface_z = inteface_mach_number(q_L_x,q_R_x,q_L_y,q_R_y,q_L_z,q_R_z,h_L_x,h_R_x,h_L_y,h_R_y,h_L_z,h_R_z,gamma_L_x,gamma_R_x,gamma_L_y,gamma_R_y,gamma_L_z,gamma_R_z)
    phi_L_x = jnp.concatenate([rho_L_x,rho_L_x*q_L_x[1:4],
                             rho_L_x*h_L_x+0.5*rho_L_x*(q_L_x[1:2]**2+q_L_x[2:3]**2+q_L_x[3:4]**2),rho_L_x*Y_L_x],axis=0)
    phi_R_x = jnp.concatenate([rho_R_x,rho_R_x*q_R_x[1:4],
                             rho_R_x*h_R_x+0.5*rho_R_x*(q_R_x[1:2]**2+q_R_x[2:3]**2+q_R_x[3:4]**2),rho_R_x*Y_R_x],axis=0)
    phi_L_y = jnp.concatenate([rho_L_y,rho_L_y*q_L_y[1:4],
                             rho_L_y*h_L_y+0.5*rho_L_y*(q_L_y[1:2]**2+q_L_y[2:3]**2+q_L_y[3:4]**2),rho_L_y*Y_L_y],axis=0)
    phi_R_y = jnp.concatenate([rho_R_y,rho_R_y*q_R_y[1:4],
                             rho_R_y*h_R_y+0.5*rho_R_y*(q_R_y[1:2]**2+q_R_y[2:3]**2+q_R_y[3:4]**2),rho_R_y*Y_R_y],axis=0)
    phi_L_z = jnp.concatenate([rho_L_z,rho_L_z*q_L_z[1:4],
                             rho_L_z*h_L_z+0.5*rho_L_z*(q_L_z[1:2]**2+q_L_z[2:3]**2+q_L_z[3:4]**2),rho_L_z*Y_L_z],axis=0)
    phi_R_z = jnp.concatenate([rho_R_z,rho_R_z*q_R_z[1:4],
                             rho_R_z*h_R_z+0.5*rho_R_z*(q_R_z[1:2]**2+q_R_z[2:3]**2+q_R_z[3:4]**2),rho_R_z*Y_R_z],axis=0)	
	#get x-flux
    ...
    #get z-flux
    M_L_plus,_,P_L_plus,_ = split_M_and_P(M_L_z)
    _,M_R_minus,_,P_R_minus = split_M_and_P(M_R_z)
    p_s = P_L_plus*p_L_z + P_R_minus*p_R_z
    w = w_pL_pR(p_L_z,p_R_z)
   	#fL,fR = fLR(p1_L_y,p1_R_y,p_L_y,p_R_y,p2_L_y,p2_R_y,p_s)
    fL,fR = fLR(M_L_z,M_R_z,p_L_z,p_R_z,p_s)
    m_half = M_L_plus + M_R_minus
    indicator = m_half>=0
    M_L_plus_av = jnp.where(indicator,M_L_plus+M_R_minus*((1-w)*(1+fR)-fL),M_L_plus*w*(1+fL))
    M_R_minus_av = jnp.where(indicator,M_R_minus*w*(1+fR),M_R_minus+M_L_plus*((1-w)*(1+fL)-fR))
    P_L = jnp.zeros_like(phi_L_z).at[3:4].set(p_L_z)
    P_R = jnp.zeros_like(phi_R_z).at[3:4].set(p_R_z)
    H_interface = M_L_plus_av*c_interface_z*phi_L_z + M_R_minus_av*c_interface_z*phi_R_z + (P_L_plus*P_L + P_R_minus*P_R)
		
    return F_interface,G_interface,H_interface
